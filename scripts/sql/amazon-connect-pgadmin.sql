-- Run this entire file in the Query Tool for the existing Rivertown PSA database.
-- No credentials or tenant IDs need to be pasted into this migration.
BEGIN;
...
-- All four entries should be non-null.
SELECT to_regclass('public.connect_credentials') AS credentials,
       to_regclass('public.connect_identities') AS identities,
       to_regclass('public.connect_conversations') AS conversations,
       to_regclass('public.connect_operations') AS operations;
